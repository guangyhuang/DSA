...
from image_synthesis.modeling.codecs.image_codec.base_function import DiffEncoder, MultiHeadAttnBlock, MHSA
from image_synthesis.modeling.codecs.image_codec.deform_conv import ModulatedDeformConvPack, modulated_deform_conv
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        if self.training and self.embed_ema and False:
            # import pdb; pdb.set_trace()
...
class PatchVQGAN(BaseCodec):
    def __init__(self,
        ...
        self.MHCA1 = MultiHeadAttnBlock(in_channels=256, head_size=8)
        self.MHSA1 = MHSA(256, 8)
        self.MHCA2 = MultiHeadAttnBlock(in_channels=256, head_size=8)
        self.MHSA2 = MHSA(256, 8)
        self.MHCA3 = MultiHeadAttnBlock(in_channels=256, head_size=8)
        self.MHSA3 = MHSA(256, 8)
        self.encoder = DiffEncoder(3)
        self.fa1 = Feature_Alignment(256)
        self.fa2 = Feature_Alignment(256)
        self.fa3 = Feature_Alignment(256)
    ...
    def get_tokens(self, data, mask=None, erase_mask=None, topk=1, return_token_index=False, cache=True, **kwargs):
        ...
        """
        data = self.pre_process(data)
        x_i = self.encoder(data, erase_mask)
        x = self.quant_conv(x_i)
        token_shape = (x.shape[-2], x.shape[-1])
        ...
        quant = self.quantize(x, token_type=token_type_erase, topk=topk)
        q = quant['quantize']
        idx = quant['index']
        q = self.post_quant_conv(q)

        x1 = self.MHCA1(x_i, q)
        x2 = self.MHSA1(x_i, erase_mask)
        x_fa1 = self.fa1(x1, x2)

        x1 = self.MHCA2(x_fa1, q)
        x2 = self.MHSA2(x_fa1, erase_mask)
        x_fa2 = self.fa2(x1, x2)

        x1 = self.MHCA3(x_fa2, q)
        x2 = self.MHSA3(x_fa2, erase_mask)
        x_r = self.fa3(x1, x2)
    ...
    @torch.no_grad()
    def decode(self, token, x, mask_im=None, mask=None, er_mask=None, combine_rec_and_gt=True, token_shape=None):
        """
        ...
        if self.decoder.requires_image:
            if mask_im is None:
                rec = self.decoder(x, self.mask_im_tmp, mask=self.mask_tmp)
            else:
                rec = self.decoder(x, self.pre_process(mask_im), mask=mask)
    ...
    def sample(self, batch):
        ...
        quant = self.quantize(x, token_type=token_type_erase, step=None, total_steps=None)
        q = quant['quantize']
        q = self.post_quant_conv(q)

        x1 = self.MHCA1(x_i, q)
        x2 = self.MHSA1(x_i, mask)
        x_fa1 = self.fa1(x1, x2)

        x1 = self.MHCA2(x_fa1, q)
        x2 = self.MHSA2(x_fa1, mask)
        x_fa2 = self.fa2(x1, x2)

        x1 = self.MHCA3(x_fa2, q)
        x2 = self.MHSA3(x_fa2, mask)
        x_r = self.fa3(x1, x2)

        rec = self.decoder(x_r)
        rec = self.post_process(rec)
        ...
        if name is None or name == 'none':
            ...
            if name == 'generator':
                params = list(self.encoder.parameters()) + \
                         ...
                         list(self.MHCA1.parameters()) + \
                         list(self.MHSA1.parameters()) + \
                         list(self.MHCA2.parameters()) + \
                         list(self.MHSA2.parameters()) + \
                         list(self.MHCA3.parameters()) + \
                         list(self.MHSA3.parameters()) + \
                         list(self.fa1.parameters()) + \
                         list(self.fa2.parameters()) + \
                         list(self.fa3.parameters()) + \
...
class Feature_Alignment (nn.Module):
    def __init__(self, channel):
        ...
        self.conv1 = nn.Conv2d(1024, 1, 1, 1)
        ...
        self.dcn = DCNv2Pack(channel, channel, 3, padding=1, deformable_groups=4)



    def forward(self, f1, f2): #16*256*32*32
        B, C, H, W = f1.size()
        f1_ = f1.view(B, C, H*W)
        f2_ = f2.view(B, C, H*W)
        att = torch.matmul(f1_.transpose(1, 2), f2_) #16*1024*1024
        ...
        w = F.softmax(w, dim=1)
        f_add = w[:, :1, :, :] * f1 + w[:, 1:, :, :] * f2  #16*256*32*32

        offset = self.offset_conv1(f_add)

        f = self.dcn(f_add, offset)
