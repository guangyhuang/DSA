...
parser.add_argument('--path1', type=str,
                    default=r'D:\A_image_inpainting\code\A_sec\PUT-33\scripts\RESULT\pl2\places2-c_val_e14-60\gt',
...
parser.add_argument('--type', type=str, default='perceptron',
...
parser.add_argument('--path2', type=str,
                    default=r'D:\A_image_inpainting\code\A_sec\PUT-33\scripts\RESULT\pl2\places2-c_val_e14-60\rec',
                    help='Paths to the images')
...
def calculate_lpips_value_diversity(path, device, net, loops, count=None, im_size=None):
    ...
    save_path = os.path.join(path, '..',
                             'lpips_{}_subd_{}_count{}_loops{}_diversity.json'.format(last_dir, len(sub_dirs), count,
                                                                                      loops))
...
def calculate_lpips_value_perceptron(path1, path2, device, net, count=None, im_size=None):
    ...
    assert len(files1) == len(files2), 'Images in two directories are not the same! {}, {}'.format(len(files1),
                                                                                                   len(files2))
    ...
    save_path = os.path.join(path2, '..',
                             'lpips_{}_count{}_perceptron.json'.format(path2.split(os.path.sep)[-1], count))
    save_dict_to_json(statics, save_path, indent=4)
    print('saved to {}'.format(save_path))
    return statics
