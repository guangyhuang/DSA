...
def count_parameters(model):
    ...
    total_params_MB = total_params * model.parameters().__next__().element_size() / (1000000)  # 转换为 MB
...
def get_model_and_dataset(args=None, model_name='2020-11-09T13-33-36_faceshq_vqgan'):
    if os.path.isfile(model_name):
        ...
        model_path = os.path.join('OUTPUT', model_name, 'checkpoint', '000149e_139649iter.pth')
    ...
    model_parameters_info = count_parameters(model)
    print("Model parameters info:", model_parameters_info)

    image = torch.randn(1, 3, 256, 256)
    image_mask = torch.randn(1, 3, 256, 256)
    erase_mask = torch.randn(1, 1, 256, 256)
    ...
    # print(summary(model, batch, show_input=True, show_hierarchical=True))
    ...
    for i, data_i in enumerate(dataloader):
        ...
        with torch.no_grad():
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                ...
                input = data_i['image_mask'].to(model.device)              ################################################################################测试
                mask = data_i.get('erase_mask', None)
                if mask is not None:
                    mask = mask.to(model.device)
                token, x = model.get_tokens(input, erase_mask = mask, return_token_index=True, cache=False)

                rec = model.decode(token['token'], x, mask_im=input, er_mask=mask, combine_rec_and_gt=False, token_shape=token.get('token_shape', None))
           # save tokens
...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='RESULT/street',
                        help='directory to save results') 
    parser.add_argument('--name', type=str, default='150-street',
    ...
    parser.add_argument('--mask_dir', type=str, default=r'D:\dataset\mask_divide\eval',
