...
class ImageListDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if data_root == '':
            data_root = r'D:\datasets\street'
            # data_root = r'D:\datasets\ffhq'
            # data_root = r'D:\datasets\places2'
    ...
    def get_mask(self, im_size, erase_mask=False, index=None):
        if self.masks is not None and random.random() < self.use_provided_mask:
            if self.image_mask_paired:
                ...
                self.current_mask_index = (self.current_mask_index + 1) % len(self.masks)  # 循环到开始，确保索引不越界
            #     index = random.randint(0, len(self.masks)-1)
            # mask = self.masks[index]['image']
            mask = self.masks[self.current_mask_index]['image']
    ...
    def __getitem__(self, index):
        ...
        if random.random() < self.erase_image_with_mask:
            mask_ = self.get_mask(im_size=(data['image'].shape[1], data['image'].shape[2]), erase_mask=True) # H W 1 #读取掩码
            mask_ = np.transpose(mask_.astype(data['image'].dtype), (2, 0, 1)) # 1 x H x W
            data['image_mask'] = mask_ * data['image']
