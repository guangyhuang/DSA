...
from image_synthesis.modeling.codecs.image_codec.base_function import DiffEncoder
# from image_synthesis.modeling.codecs.image_codec.base_function import SelfAttention
# from image_synthesis.modeling.codecs.image_codec.base_function import ContextualAttention

def attention(x, token_type_erase):
    batch_size, _, height, width = x.shape
    complete_region = x * token_type_erase
    missing_region = x * (1 - token_type_erase)

    x_c = complete_region.permute(0, 2, 3, 1).contiguous()  # B x H x W x C
    x_c = x_c.view(batch_size, -1, 256)  # BHW x C

    x_m = missing_region.permute(0, 2, 3, 1).contiguous()  # B x H x W x C
    x_m = x_m.view(batch_size, -1, 256)  # BHW x C

    attn_weights = torch.matmul(x_m, x_c.transpose(1, 2))
    mask = torch.where(token_type_erase<1, -torch.tensor(float('inf')).to('cuda'),torch.tensor(1.0).to('cuda'))
    mask = mask.view(batch_size,-1,height*width)
    attn_weights = mask + attn_weights
    attn_weights = nn.functional.softmax(attn_weights, dim=-1)
    x1 = torch.matmul(attn_weights, x_c)
    x1 = x1.view(batch_size, height, width, 256)
    x1 = x1.permute(0, 3, 1, 2).contiguous()
    x1 = x1 * (1 - token_type_erase)
    x = x1 + complete_region
    return x



def Classification(images, mask):
    x_c = []
    x_m = []
...
class VectorQuantizer(nn.Module):
    ...
    def get_distance(self, z, code_type='unmasked'):
        """
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        # # 如果没有指定 token_type 或者没有蒙版编码，直接获取最小距离的索引
        ...
        if token_type is None:
            ...
            if idx.sum() > 0:
                d_ = d[idx][:, :self.masked_embed_start] # l x n
                # indices_ = torch.argmin(d_, dim=1)
                indices_c = self.get_index(d_, topk=topk, step=step, total_steps=total_steps)
                min_encoding_indices[idx] = indices_c
        ...
        if self.get_embed_type == 'matmul':
            ...
            # features_numpy = self.embed_weight.t().cpu().numpy()
            #
            # plt.figure(figsize=(10, 20))
            # plt.imshow(features_numpy, cmap='viridis', aspect='auto')
            # plt.colorbar()
            # plt.title("Visualization of the Features")
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        idx = token_type_flattened == 1
        z_c = z_flattened[idx]
        z_cq, min_encoding_indices = self._quantize(z_c, token_type=None, topk=topk, step=step, total_steps=total_steps)
        ...
        if self.training and self.embed_ema:
            ...
            z_sum = (z_c.transpose(0, 1) @ indices_onehot).transpose(0, 1) # n_e x D
        ...
        z_q = z_flattened.clone()
        z_q[idx] = z_cq
        z_q = z_q.view(batch_size, height, width, -1)
        if self.embed_ema:
        ...
        #     return output
        # else:
        #     return output['index'], z_flattened, idx
...
class PatchVQGAN(BaseCodec):
    def __init__(self,
        ...
        if conv_before_quantize:
            self.quant_conv = torch.nn.Conv2d(256, self.quantize.e_dim, 1)
        else:
    ...
    def get_tokens(self, data, mask=None, erase_mask=None, topk=1, return_token_index=False, cache=True, **kwargs):
        ...
        """
        data = self.pre_process(data)
        x = self.encoder(data, erase_mask)
        x = self.quant_conv(x)
        token_shape = (x.shape[-2], x.shape[-1])
        ...
        m = F.interpolate(erase_mask, size=[32,32], mode='nearest')
        x = attention(x, m)

        idx, x_f, mask_idx = self.quantize(x, token_type=token_type_erase, topk=topk)
        ...
        output['token_shape'] = token_shape
        return output, x_f, mask_idx, token_type_erase

    @torch.no_grad()
    def decode(self, token, x, idx,mask_im=None, mask=None, er_mask=None, combine_rec_and_gt=True, token_shape=None):
        """
        ...
        quant = self.quantize.get_codebook_entry(token.view(-1), None)
        ...
        x1[idx] = quant
        x1 = x1.view(-1, 32, 32, 256)
        x1 = x1.permute(0, 3, 1, 2).contiguous()
        x1 = self.post_quant_conv(x1)
        m = F.interpolate(er_mask, size=[32,32], mode='nearest')
        x1 = attention(x1, m)

        weight_map = self.weight_calc(x1)
        quant = weight_map[:, 0:1, :, :] * x + weight_map[:, 1:2, :, :] * x1
        ...
        # import pdb; pdb.set_trace()
        # rec = rec * (1 - er_mask.to(rec.dtype)) + self.pre_process(mask_im) * er_mask.to(rec.dtype)
    ...
    def sample(self, batch):
        ...
        # 编码器
        data = self.pre_process(batch['image_mask'])
        mask = batch['erase_mask'].cuda()
        x = self.encoder(data, mask)
        x = self.quant_conv(x)
        token_shape = (x.shape[-2], x.shape[-1])
        ...
        # 量化后
        quant = attention(quant, m)
        weight_map = self.weight_calc(quant)
        quant_add = weight_map[:, 0:1, :, :] * x + weight_map[:, 1:2, :, :] * quant

        rec = self.decoder(quant_add)
        rec = self.post_process(rec)

        out = {'input': batch['image_mask'], 'reconstruction': rec}
        ...
        if name is None or name == 'none':
            ...
            if name == 'generator':
                params = list(self.encoder.parameters())+ \
                         ...
                         list(self.weight_calc.parameters())+\
    ...
    def forward(self, batch, name='none', return_loss=True, step=0, total_steps=None, **kwargs):
        ...
        if name == 'generator':
            gt = self.pre_process(batch['image'])
            input = self.pre_process(batch['image_mask'])
            mask = batch['erase_mask']

            x = self.encoder(input, mask)
            x = self.quant_conv(x)
