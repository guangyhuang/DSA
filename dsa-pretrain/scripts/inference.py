    ...
    for i, data_i in enumerate(dataloader):
        ...
        with torch.no_grad():
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                ...
                token, x_f, mask_idx, token_type_erase  = model.get_tokens(input, erase_mask = mask, return_token_index=True, cache=False)
                # rec = model.decode(token['token'], x=x_f, idx=mask_idx, mask=token_type_erase, combine_rec_and_gt=False, token_shape=token.get('token_shape', None))
                rec = model.decode(token['token'], x=x_f, idx=mask_idx, mask=token_type_erase, mask_im=input, er_mask=mask,
                               combine_rec_and_gt=True, token_shape=token.get('token_shape', None))
           # save tokens
...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='pvqvae_ffhq_20',
    ...
    # parser.add_argument('--name', type=str, default='transformer_naturalscene',
    #                     help='the name of this experiment, if not provided, set to'
    #                          'the name of config file')
    # parser.add_argument('--func', type=str, default='inference_complet_sample_in_feature_for_evaluation', #TODO'',
    #                     help='the name of inference function')
    # args for ddp
